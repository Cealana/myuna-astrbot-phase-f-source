...
class ExternalContextEnvelope:
    ...
    def __post_init__(self) -> None:
        if self.visual_evidence is None:
            ...
            if self.projection_policy_version != EXTERNAL_PROJECTION_POLICY:
    ...
    def _validate_chain(self) -> None:
        if self.recent_turns:
            ...
            expected_sequence = self.summary.covered_end + 1 if self.summary else 1
            expected_parent = (
                self.summary.covered_terminal_digest if self.summary else ZERO_DIGEST
            )
    ...
    def from_payload(
        ...
        *,
        ...
        if len(turns_payload) > MAX_RECENT_TURNS:
            raise ExternalContextError("recent_turn_count_exceeded")
