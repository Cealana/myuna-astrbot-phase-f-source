...
class ReleaseBoundExternalContext:
    ...
    schema: str = RELEASE_BOUND_CONTEXT_SCHEMA

    def __post_init__(self) -> None:
        _require(self.schema == RELEASE_BOUND_CONTEXT_SCHEMA, "release_bound_context_schema_unknown")
...
class ReleaseBoundTurnProvenance:
    ...
    schema: str = RELEASE_BOUND_PROVENANCE_SCHEMA

    def __post_init__(self) -> None:
        _require(self.schema == RELEASE_BOUND_PROVENANCE_SCHEMA, "release_bound_provenance_schema_unknown")
    ...
    def from_payload(cls, payload: object) -> "ReleaseBoundTurnProvenance":
        ...
        _require(isinstance(payload, Mapping) and set(payload) == required, "release_bound_provenance_fields_rejected")
        return cls(
            release_set_id=payload["release_set_id"],  # type: ignore[arg-type]
