...
from gateway_degradation_protocol import (
    ...
    validate_core_failure_response,
...
class CoreUnavailable(RuntimeError):
    ...
    def __init__(self, projection: object, *, projection_source: str) -> None:
...
class LoopbackCoreClient:
    ...
    def chat(self, messages: list[dict[str, str]]) -> CoreReply:
        body = json.dumps(
            {
                "high_quality": False,
                "messages": messages,
        ...
        if response_status != 200:
            try:
                projection = validate_core_failure_response(response_status, payload)
            except GatewayDegradationProtocolError:
                raise CoreUnavailable(
                    deterministic_gateway_projection("gateway-core-invalid-response"),
                    projection_source="gateway",
                ) from None
            raise CoreUnavailable(projection, projection_source="core")
...
def _degradation_fanout(
    ...
    *,
    ...
    try:
        ...
        return PostConnectionFanout(degradation=job)
