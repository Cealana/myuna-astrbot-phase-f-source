...
def _respond_stage_for_outcome_test() -> RespondStage:
    stage = RespondStage()
...
@pytest.mark.asyncio
async def test_respond_stage_emits_content_free_success_outcome(
    astr_message_event,
):
    astr_message_event.set_result(MessageEventResult().message("synthetic reply"))
    astr_message_event.send = AsyncMock(return_value=None)
    hook = AsyncMock(return_value=False)
    with patch("astrbot.core.pipeline.respond.stage.call_event_hook", hook):
        await _respond_stage_for_outcome_test().process(astr_message_event)
    assert astr_message_event.get_extra("_astrbot_send_outcome_v1") == (
        1,
        "succeeded",
        1,
        1,
...
@pytest.mark.asyncio
async def test_respond_stage_emits_one_failed_outcome_after_send_exception(
    astr_message_event,
):
    sentinel = "private-exception-sentinel"
    astr_message_event.set_result(MessageEventResult().message("synthetic reply"))
    astr_message_event.send = AsyncMock(side_effect=RuntimeError(sentinel))
    hook = AsyncMock(return_value=False)
    with patch("astrbot.core.pipeline.respond.stage.call_event_hook", hook):
        await _respond_stage_for_outcome_test().process(astr_message_event)
    outcome = astr_message_event.get_extra("_astrbot_send_outcome_v1")
    assert outcome == (1, "failed", 1, 0)
    assert sentinel not in repr(outcome)
    hook.assert_awaited_once()


@pytest.mark.asyncio
async def test_respond_stage_no_send_does_not_fabricate_success(
    astr_message_event,
):
    astr_message_event.set_result(MessageEventResult())
    hook = AsyncMock(return_value=False)
    with patch("astrbot.core.pipeline.respond.stage.call_event_hook", hook):
        await _respond_stage_for_outcome_test().process(astr_message_event)
    assert astr_message_event.get_extra("_astrbot_send_outcome_v1") is None
    hook.assert_not_awaited()


@pytest.mark.asyncio
async def test_respond_stage_aggregates_record_and_partial_failure(
    astr_message_event,
):
    result = MessageEventResult().message("synthetic plain")
    result.chain.append(Record(file="synthetic-record.wav"))
    astr_message_event.set_result(result)
    astr_message_event.send = AsyncMock(
        side_effect=[None, RuntimeError("private-partial-sentinel")]
    )
    hook = AsyncMock(return_value=False)
    with patch("astrbot.core.pipeline.respond.stage.call_event_hook", hook):
        await _respond_stage_for_outcome_test().process(astr_message_event)
...
@pytest.mark.asyncio
async def test_respond_stage_aggregates_segmented_sends(
    astr_message_event,
):
    stage = _respond_stage_for_outcome_test()
    stage.enable_seg = True
    stage.only_llm_result = False

    async def no_delay(_component):
        return 0

    stage._calc_comp_interval = no_delay
    result = MessageEventResult().message("first")
    result.chain.append(Plain(text="second"))
    astr_message_event.set_result(result)
    astr_message_event.send = AsyncMock(return_value=None)
    hook = AsyncMock(return_value=False)
    with patch("astrbot.core.pipeline.respond.stage.call_event_hook", hook):
