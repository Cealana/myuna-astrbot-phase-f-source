...
# Modified by Myuna on 2026-08-21: aggregate a content-free send outcome for
# the existing post-send hook. AstrBot remains AGPL-3.0-or-later.
...
class RespondStage(Stage):
    ...
    async def process(
        ...
        status = (
            "succeeded"
            if succeeded_count == attempted_count
            else "failed"
        )
        event.set_extra(
            "_astrbot_send_outcome_v1",
            (1, status, attempted_count, succeeded_count),
        )
