...
IDENT_MAPPING="myuna_dev_map    myuna-gateway-telegram    myuna_telegram_gateway_app"

if [[ ! -d $CONFIG_DIR ]]; then
    echo "PostgreSQL cluster config is unavailable" >&2
    exit 1
fi
if ! getent passwd myuna-gateway-telegram >/dev/null; then
    echo "Telegram gateway service account is unavailable" >&2
    exit 1
fi
if ! runuser -u postgres -- psql --dbname myuna_dev --no-psqlrc --tuples-only \
    ...
    | grep -Fxq 1; then
    echo "Telegram gateway database role is unavailable" >&2
    exit 1
fi
...
peer_user=$(runuser -u myuna-gateway-telegram -- psql \
    --dbname myuna_dev \
...
if [[ $peer_user != myuna_telegram_gateway_app ]]; then
    echo "Telegram gateway peer authentication verification failed" >&2
    exit 1
fi

echo "Telegram gateway Unix peer authentication installed and verified"
