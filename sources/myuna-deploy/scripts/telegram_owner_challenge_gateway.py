...
CONFIG_PATH = Path("/etc/myuna-telegram-gateway/owner-challenge-v1.json")
EVIDENCE_PATH = Path(
    "/var/lib/myuna-telegram-gateway/owner-challenge-v1-evidence.json"
)
...
def _audit_stage(code: str) -> None:
    ...
    print(f"telegram owner challenge gateway stage={code}", flush=True)
    ...
    def from_payload(cls, payload: object) -> "ChallengeConfig":
        ...
        for key in ("binding_id", "namespace_id", "principal_id"):
            if not isinstance(payload[key], str) or _SAFE_ID.fullmatch(payload[key]) is None:
                raise ChallengeRejected("challenge rejected")
        if (
            not isinstance(payload["channel_instance"], str)
            or _SAFE_ID.fullmatch(payload["channel_instance"]) is None
        ):
            raise ChallengeRejected("challenge rejected")
...
def write_evidence(config: ChallengeConfig, decision: ChallengeDecision, now: datetime) -> None:
    ...
    evidence = {
        ...
        "result": "telegram-private-challenge-matched",
