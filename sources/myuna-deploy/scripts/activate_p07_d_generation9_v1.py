...
from p07_d_generation9_release_set import (
    GENERATION,
    build_release_set,
    canonical,
    digest,
...
RELEASE_SET_EPOCH_ID = RELEASE_SET_EPOCH_ID_9
RELEASE_SET_EPOCH_PATH = RELEASE_SET_EPOCH_PATH_9


SCHEMA = "myuna.p07-d-generation9-activation.v1"
...
BACKUP_ROOT = Path("/var/backups/myuna/p07-d-generation9-activation-v1")
...
class Generation9ActivationRejected(RuntimeError):
...
def _failure_projection(exc: Exception) -> dict[str, str]:
    ...
    if not isinstance(final_gate, str) or _TYPED_FAILURE_GATE.fullmatch(final_gate) is None:
        final_gate = "generation9_activation_rejected"
...
def require(condition: bool, code: str) -> None:
    if not condition:
        raise Generation9ActivationRejected(code)
...
def _load_b_selector() -> tuple[bytes, dict[str, object]]:
    ...
    try:
        ...
        raise Generation9ActivationRejected("b_selector_document_rejected") from exc
...
def prepare_activation(
    ...
    *,
    ...
    require(not Path(RELEASE_SET_EPOCH_PATH).parent.exists() and not Path(RELEASE_SET_EPOCH_PATH).parent.is_symlink(), "generation9_epoch_preexisting")
    ...
    plan_preimage = {
        ...
        "prestate_digest": digest("myuna-p07-d-generation9-prestate-v1", prestate),
    ...
    with tempfile.TemporaryDirectory(prefix="p07-d-release-set-smoke-") as directory:
        ...
        for index, (user, pythonpath, module, function) in enumerate(probes):
            program = (
                ...
                "assert getattr(v,'release_set',v).generation==9"
            )
...
class Generation9LiveBackend:
    ...
    def capture_prestate(self) -> ActivationPrestate:
        ...
        return ActivationPrestate(
            state_digest=digest("myuna-p07-d-generation9-prestate-observation-v1", self._functional_payload(self.prestate_observation)),
    ...
    def verify_rollback_ready(self, prestate: ActivationPrestate) -> None:
        require(prestate.state_digest == digest("myuna-p07-d-generation9-prestate-observation-v1", self._functional_payload(self.prestate_observation)), "rollback_prestate_digest_rejected")
    ...
    def apply_target_release_set(self, snapshot: ProtectedReleaseSetSnapshot) -> None:
        ...
        require(not target_parent.exists() and not target_parent.is_symlink(), "generation9_epoch_preexisting")
...
def activate(prepared: PreparedActivation, *, expected_plan_sha256: str | None, preflight_only: bool) -> dict[str, object]:
    ...
    if phase_f_selected_target(Path(__file__).resolve().parent):
        raise Generation9ActivationRejected("phase_f_canonical_owner_required")
    ...
    try:
        result = AtomicReleaseSetTransaction(Generation9LiveBackend(prepared, backup_root)).run()
        receipt = {
            "attempt": attempt,
