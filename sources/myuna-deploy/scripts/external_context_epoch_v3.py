...
class ExternalEpochV3Store:
    def __init__(
        ...
        *,
        ...
        _require(
            ...
            "epoch_v3_projection_policy_rejected",
        )
        self.projection_policy_version = projection_policy_version
        if projection_policy_version == EXTERNAL_VERBATIM_FIRST_PROJECTION_POLICY:
            _require(
                isinstance(policy_overlay_id, str)
                and _TOKEN.fullmatch(policy_overlay_id) is not None,
                "epoch_v3_policy_overlay_rejected",
            )
        else:
            _require(
                policy_overlay_id is None,
                "epoch_v3_policy_overlay_rejected",
            )
        self.policy_overlay_id = policy_overlay_id
    ...
    def inspect_existing_metadata(
        ...
        *,
        ...
        store.projection_policy_version = EXTERNAL_PROJECTION_POLICY
        store.policy_overlay_id = None
    ...
    def projection_readiness(
        ...
        """Return a content-free, non-mutating visual-egress readiness result."""
        ...
        try:
            with self._connect() as connection:
                ...
                if connection.execute(
                    "SELECT 1 FROM pending_turns LIMIT 1"
                ).fetchone() or connection.execute(
                    "SELECT 1 FROM delivery_intents LIMIT 1"
                ).fetchone():
                ...
                aggregate = connection.execute(
                    ...
                    SELECT COUNT(*) AS turn_count,
                           COALESCE(SUM(LENGTH(user_message) +
                                        LENGTH(assistant_reply)), 0) AS characters
                ...
                if (
                    aggregate["turn_count"] > MAX_RECENT_TURNS
                    or aggregate["characters"] > MAX_RECENT_CHARACTERS
                ):
        ...
        *,
        ...
        try:
            with self._connect() as connection:
                ...
                summary = self._active_summary(connection, state)
                use_verbatim = (
                    visual_event is None
                    and self.projection_policy_version
                    == EXTERNAL_VERBATIM_FIRST_PROJECTION_POLICY
                )
                first = 1 if use_verbatim or summary is None else summary.covered_end + 1
                rows = connection.execute(
                    "SELECT * FROM committed_turns WHERE sequence BETWEEN ? AND ? ORDER BY sequence",
                    (first, state["latest_sequence"]),
                ).fetchall()
                if use_verbatim:
                    verbatim_characters = sum(
                        len(row["user_message"]) + len(row["assistant_reply"])
                        for row in rows
                    )
                    if (
                        len(rows) > MAX_VERBATIM_RECENT_TURNS
                        or verbatim_characters > MAX_VERBATIM_RECENT_CHARACTERS
                    ):
                        _require(
                            summary is not None,
                            "external_summary_backpressure",
                        )
                        first = summary.covered_end + 1
                        rows = connection.execute(
                            "SELECT * FROM committed_turns WHERE sequence BETWEEN ? AND ? ORDER BY sequence",
                            (first, state["latest_sequence"]),
                        ).fetchall()
                ...
                _require(
                    (
                        use_verbatim
                        and first == 1
                        and len(turns) <= MAX_VERBATIM_RECENT_TURNS
                        and sum(
                ...
                envelope = ExternalContextEnvelope(
                    ...
                    projection_policy_version=(
                        ...
                        else self.projection_policy_version
                    ),
                    schema=(
                        EXTERNAL_VISUAL_CONTEXT_SCHEMA
                        if visual_evidence is not None
    ...
    def prepare_delivery(
        ...
        *,
        ...
        _require(
            provenance.policy_overlay_id == self.policy_overlay_id,
