...
def _orchestrate_product(
    ...
    deploy_tree = _validate_repository(deploy_root, deploy_commit, parent=product.ACCEPTED_DEPLOY_PARENT)
    ...
    source = {"core_commit": core_commit, "core_tree": core_tree, "deploy_commit": deploy_commit, "deploy_parent": product.ACCEPTED_DEPLOY_PARENT, "deploy_tree": deploy_tree}
...
def verify_release(output_root: Path, digest: str, expected_authority: Mapping[str, object]) -> bool:
    ...
    except (KeyError, OSError, boot.ResumeRejected):
        return False
    return True
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(json.dumps({"release_digest": digest, "status": "built"}, separators=(",", ":"), sort_keys=True))
    return 0
