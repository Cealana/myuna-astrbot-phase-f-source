...
class ExternalEpochStore:
    ...
    def projection_readiness(
        ...
        """Return a content-free pre-egress capacity result without mutation."""
        ...
        try:
            with self._connect() as connection:
                ...
                revision = connection.execute(
                    "SELECT selected_sequence, summary_version "
                    "FROM epoch_revisions WHERE revision = ?",
                    (state["selected_revision"],),
                ).fetchone()
                if revision is None:
                ...
                if revision["summary_version"] is not None:
                    summary = connection.execute(
                        "SELECT covered_end FROM committed_summaries "
