...
def _assistant_text(turn: int) -> str:
    marker = "TAIL=月桂玻璃" if turn == 64 else f"ASSISTANT-{turn:02d}"
...
def run_offline_gate() -> dict[str, object]:
    ...
    checks = {
        "stored_exactly_128": len(stored) == PROFILE_MAX_MESSAGES,
        "request_exactly_127": len(request) == SATURATED_REQUEST_MESSAGES,
        ...
        "first_retained_present": any("FIRST=雾蓝纸鸢" in item for item in contents),
        ...
        "cjk_body_fits_candidate_limit": (
            cjk_body_bytes <= PROFILE_HTTP_MAX_BODY_BYTES
        ),
        "escaped_body_fits_candidate_limit": (
            escaped_body_bytes <= PROFILE_HTTP_MAX_BODY_BYTES
        ),
        "initial_complete_input_fits_candidate_limit": (
            PROFILE_DEFINITION_MAX_CHARACTERS + PROFILE_MAX_CHARACTERS
            <= PROFILE_MODEL_INPUT_MAX_CHARACTERS
        ),
        "repair_headroom_at_least_65536": (
            PROFILE_MODEL_INPUT_MAX_CHARACTERS
    ...
    return {
        "schema": "myuna.context-capacity-128.offline-gate.v1",
        "profile": {
            "max_history_messages": PROFILE_MAX_MESSAGES,
            "max_history_characters": PROFILE_MAX_CHARACTERS,
            "http_max_body_bytes": PROFILE_HTTP_MAX_BODY_BYTES,
            "model_input_max_characters": PROFILE_MODEL_INPUT_MAX_CHARACTERS,
        ...
        "measurements": {
            ...
            "initial_complete_input_characters": (
                PROFILE_DEFINITION_MAX_CHARACTERS + PROFILE_MAX_CHARACTERS
            ),
            "repair_headroom_characters": (
                PROFILE_MODEL_INPUT_MAX_CHARACTERS
                - PROFILE_DEFINITION_MAX_CHARACTERS
                - PROFILE_MAX_CHARACTERS
            ),
