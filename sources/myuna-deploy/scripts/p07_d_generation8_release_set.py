...
from myuna_core.external_context.release_set import (
    ...
    RELEASE_SET_GENERATION_8,
)


GENERATION = RELEASE_SET_GENERATION_8
...
class Generation8ReleaseSetRejected(RuntimeError):
...
def _require(condition: bool, code: str) -> None:
    if not condition:
        raise Generation8ReleaseSetRejected(code)
...
def digest(domain: str, value: object) -> str:
    _require(bool(domain) and domain.isascii(), "generation8_digest_domain_rejected")
...
def selector_payload(previous_bundle_digest: str) -> dict[str, object]:
    _require(_SHA256.fullmatch(previous_bundle_digest) is not None, "generation8_bundle_digest_rejected")
    return {
...
def service_binding_digest(
    *,
    ...
    _require(kind in {"core", "telegram", "telegram_socket"}, "generation8_service_kind_rejected")
    _require(unit.endswith((".service", ".socket")), "generation8_service_unit_rejected")
    _require(uid >= 0 and gid >= 0, "generation8_service_identity_rejected")
    _require(_SHA256.fullmatch(release_set_acl_digest) is not None, "generation8_acl_digest_rejected")
    _require(
        bool(binding_files)
        ...
        "generation8_service_files_rejected",
    )
    return digest(
