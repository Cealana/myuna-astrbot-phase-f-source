...
GENERATION = RELEASE_SET_GENERATION_9
...
class Generation9ReleaseSetRejected(RuntimeError):
...
def _require(condition: bool, code: str) -> None:
    if not condition:
        raise Generation9ReleaseSetRejected(code)
...
def selector_payload(previous_bundle_digest: str) -> dict[str, object]:
    _require(
        ...
        "generation9_bundle_digest_rejected",
    )
    return {
