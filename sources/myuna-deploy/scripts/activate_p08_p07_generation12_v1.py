#!/usr/bin/env python3
"""Atomic generation-12 P07 + Telegram plugin + P08 coordinator.
...
from activate_p07_d_generation12_v1 import (
    Generation12ActivationRejected,
    Generation12LiveBackend,
...
from activate_p07_hybrid_external_generation_v1 import PLUGIN_ROOT
from build_telegram_gateway_release_v1 import verify_release as verify_plugin_release
from p08_p07_combined_release_set_v1 import CombinedReleaseSet
...
PLAN_SCHEMA = "myuna.p08-p07-generation12-activation-plan.v1"
JOURNAL_SCHEMA = "myuna.p08-p07-generation12-journal.v1"
RECEIPT_SCHEMA = "myuna.p08-p07-generation12-receipt.v1"
STATE_ROOT = Path("/var/lib/myuna-telegram-gateway/p08-p07-generation12-v1")
BACKUP_ROOT = Path("/var/backups/myuna/p08-p07-generation12-v1")
...
def _permissions_digest(bundle: Mapping[str, object]) -> str:
    ...
    return digest(
        "myuna-p08-p07-generation12-previous-epoch-permissions-v1",
...
def _units_digest(p08_release: Path) -> str:
    ...
    return digest("myuna-p08-generation12-unit-set-v1", rows)
    ...
    *,
    ...
    combined_prestate = {
        "p07_prestate_digest": digest(
            "myuna-p08-p07-generation12-p07-prestate-v1", p07.prestate
        ),
    ...
    combined = CombinedReleaseSet.create(
        ...
        rollback={
            "combined_prestate_digest": digest(
                "myuna-p08-p07-generation12-combined-prestate-v1",
                combined_prestate,
            ),
...
class LiveCombinedBackend:
    def __init__(self, prepared: PreparedCombinedActivation, backup_root: Path) -> None:
        ...
        self.p07_backend = Generation12LiveBackend(prepared.p07, p07_backup)
    ...
    def observe_target(self) -> str:
        ...
        return digest(
            "myuna-p08-p07-generation12-target-observation-v1",
            {
...
def _attempt_count() -> int:
    ...
    require(
        payload.get("schema") == "myuna.p08-p07-generation12-attempt-ledger.v1"
        ...
        canonical(
            {
                ...
                "schema": "myuna.p08-p07-generation12-attempt-ledger.v1",
            }
        ),
        mode=0o600,
    )
