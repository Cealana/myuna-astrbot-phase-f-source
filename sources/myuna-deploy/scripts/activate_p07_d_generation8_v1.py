#!/usr/bin/env python3
"""Rollback-bound generation-8 P07-D successor release-set activation."""
...
RELEASE_SET_EPOCH_ID = RELEASE_SET_EPOCH_ID_8
RELEASE_SET_EPOCH_PATH = RELEASE_SET_EPOCH_PATH_8


SCHEMA = "myuna.p07-d-generation8-activation.v1"
...
BACKUP_ROOT = Path("/var/backups/myuna/p07-d-generation8-activation-v1")
STATE_ROOT = Path("/var/lib/myuna-telegram-gateway/p07-d-generation8-activation-v1")
ATTEMPT_LEDGER = Path(
    "/var/lib/myuna-telegram-gateway/p07-d-generation7-activation-v1/"
    "ATTEMPT_LEDGER.json"
)
...
class Generation8ActivationRejected(RuntimeError):
...
def _failure_projection(exc: Exception) -> dict[str, str]:
    ...
    if not isinstance(final_gate, str) or _TYPED_FAILURE_GATE.fullmatch(final_gate) is None:
        final_gate = "generation8_activation_rejected"
...
def require(condition: bool, code: str) -> None:
    if not condition:
        raise Generation8ActivationRejected(code)
...
def _load_b_selector() -> tuple[bytes, dict[str, object]]:
    ...
    try:
        ...
        raise Generation8ActivationRejected("b_selector_document_rejected") from exc
...
def prepare_activation(
    ...
    *,
    ...
    require(not Path(RELEASE_SET_EPOCH_PATH).parent.exists() and not Path(RELEASE_SET_EPOCH_PATH).parent.is_symlink(), "generation8_epoch_preexisting")
    ...
    plan_preimage = {
        ...
        "prestate_digest": digest("myuna-p07-d-generation8-prestate-v1", prestate),
    ...
    with tempfile.TemporaryDirectory(prefix="p07-d-release-set-smoke-") as directory:
        ...
        for index, (user, pythonpath, module, function) in enumerate(probes):
            program = (
                ...
                "assert getattr(v,'release_set',v).generation==8"
            )
...
class Generation8LiveBackend:
    ...
    def capture_prestate(self) -> ActivationPrestate:
        ...
        return ActivationPrestate(
            state_digest=digest("myuna-p07-d-generation8-prestate-observation-v1", self._functional_payload(self.prestate_observation)),
    ...
    def verify_rollback_ready(self, prestate: ActivationPrestate) -> None:
        require(prestate.state_digest == digest("myuna-p07-d-generation8-prestate-observation-v1", self._functional_payload(self.prestate_observation)), "rollback_prestate_digest_rejected")
    ...
    def apply_target_release_set(self, snapshot: ProtectedReleaseSetSnapshot) -> None:
        ...
        require(not target_parent.exists() and not target_parent.is_symlink(), "generation8_epoch_preexisting")
...
def _attempt_count() -> int:
    ...
    require(isinstance(payload, dict) and payload.get("schema") == "myuna.p07-d-generation7-attempt-ledger.v1" and type(payload.get("attempts")) is int and 0 <= payload["attempts"] <= 2, "attempt_ledger_rejected")
...
def _consume_attempt(plan_digest: str) -> int:
    ...
    count += 1
    atomic_write(ATTEMPT_LEDGER, canonical({"attempts": count, "last_plan_sha256": plan_digest, "schema": "myuna.p07-d-generation7-attempt-ledger.v1"}), mode=0o600)
    return count
...
def activate(prepared: PreparedActivation, *, expected_plan_sha256: str | None, preflight_only: bool) -> dict[str, object]:
    ...
    if phase_f_selected_target(Path(__file__).resolve().parent):
        raise Generation8ActivationRejected("phase_f_canonical_owner_required")
    ...
    try:
        result = AtomicReleaseSetTransaction(Generation8LiveBackend(prepared, backup_root)).run()
        receipt = {
            "attempt": attempt,
