#!/usr/bin/env python3
"""Generation-12 P07 component for the combined P08/P07 transaction.
...
SCHEMA = "myuna.p07-generation12-component-activation.v1"
...
BACKUP_ROOT = Path("/var/backups/myuna/p08-p07-generation12-v1/p07-component")
STATE_ROOT = Path("/var/lib/myuna-telegram-gateway/p08-p07-generation12-v1/p07-component")
ATTEMPT_LEDGER = Path(
    "/var/lib/myuna-telegram-gateway/p08-p07-generation12-v1/p07-component/"
    "ATTEMPT_LEDGER.json"
)
...
class Generation12ActivationRejected(RuntimeError):
...
def _failure_projection(exc: Exception) -> dict[str, str]:
    ...
    if not isinstance(final_gate, str) or _TYPED_FAILURE_GATE.fullmatch(final_gate) is None:
        final_gate = "generation12_activation_rejected"
...
def require(condition: bool, code: str) -> None:
    if not condition:
        raise Generation12ActivationRejected(code)
...
def _load_previous_selector() -> tuple[bytes, dict[str, object]]:
    ...
    try:
        ...
        raise Generation12ActivationRejected("previous_selector_document_rejected") from exc
    require(
...
def prepare_activation(
    ...
    *,
    ...
    for value in (
        ...
        expected_previous_epoch_sha256,
        expected_previous_release_set_id,
    ):
        require(_DIGEST.fullmatch(value) is not None, "expected_digest_rejected")
    require(active(CORE_SERVICE) and active(TELEGRAM_SERVICE) and active(TELEGRAM_SOCKET), "previous_services_inactive")
    previous_release_set_bytes, previous_release_set = _load_previous_release_set()
    require(previous_release_set.release_set_id == expected_previous_release_set_id, "previous_release_set_drifted")
    require(previous_release_set.core["release_digest"] == expected_core_release, "previous_core_release_set_drifted")
    require(previous_release_set.telegram_runtime["release_digest"] == expected_runtime_release, "previous_runtime_release_set_drifted")
    require(show(CORE_SERVICE, "WorkingDirectory").endswith(expected_core_release), "previous_core_release_drifted")
    require(f"/{expected_runtime_release}/runtime/telegram_owner_runtime_gateway.py" in show(TELEGRAM_SERVICE, "ExecStart"), "previous_runtime_release_drifted")
    ...
    previous_selector_bytes, previous_selector = _load_previous_selector()
    require(not Path(RELEASE_SET_EPOCH_PATH).parent.exists() and not Path(RELEASE_SET_EPOCH_PATH).parent.is_symlink(), "generation12_epoch_preexisting")
    first_config = load_target_runtime_config_snapshot()
    previous_metadata = _expected_previous_metadata(
        previous_release_set=previous_release_set,
    ...
    require(database_entry["sha256"] == expected_previous_epoch_sha256, "previous_epoch_digest_drifted")
    time.sleep(0.25)
    ...
    previous_bundle_digest = require_same_bundle(first_bundle, second_bundle)
    require(
        previous_selector == generation11_selector_payload(
            str(previous_selector["previous_epoch_bundle_digest"])
        )
        and previous_release_set.selector["digest"] == digest_bytes(previous_selector_bytes),
        "previous_selector_release_set_mismatch",
    )
    ...
    selector_bytes = canonical(selector_payload(previous_bundle_digest))
    ...
    require(all(item.active_state == "active" and item.result == "success" for item in prestate_services), "previous_service_state_rejected")
    prestate = {
        "previous_bundle": second_bundle,
        "previous_epoch_metadata": previous_metadata,
        "previous_release_set_id": previous_release_set.release_set_id,
    ...
    rollback_manifest = {
        "files": prestate_files,
        "selector_digest": digest_bytes(previous_selector_bytes),
    ...
    release_set = build_release_set(
        ...
        rollback={
            ...
            "epoch_bundle_digest": previous_bundle_digest,
    ...
    plan_preimage = {
        ...
        "prestate_digest": digest("myuna-p07-generation12-prestate-v1", prestate),
...
def _restore_release_set_prestate(
    path: Path,
    payload: bytes | None,
    *,
    backup_root: Path,
    ...
    if payload is None:
        if path.exists() or path.is_symlink():
            preserved = backup_root / "ROLLED_BACK_TARGET_RELEASE_SET.json"
        ...
        return
    atomic_write(path, payload, mode=0o600, gid=0)
    projection = apply_release_set_acl(
        path,
        core_uid=core_uid,
        telegram_uid=telegram_uid,
    )
    require(projection.file_mode == 0o640, "rollback_release_set_acl_rejected")
...
class Generation12LiveBackend:
    def __init__(self, prepared: PreparedActivation, backup_root: Path) -> None:
        ...
        self.bundle_prestate = dict(prepared.prestate["previous_bundle"])
    ...
    def capture_prestate(self) -> ActivationPrestate:
        ...
        return ActivationPrestate(
            state_digest=digest("myuna-p07-generation12-prestate-observation-v1", self._functional_payload(self.prestate_observation)),
    ...
    def verify_rollback_ready(self, prestate: ActivationPrestate) -> None:
        require(prestate.state_digest == digest("myuna-p07-generation12-prestate-observation-v1", self._functional_payload(self.prestate_observation)), "rollback_prestate_digest_rejected")
    ...
    def apply_target_release_set(self, snapshot: ProtectedReleaseSetSnapshot) -> None:
        ...
        current_selector, payload = _load_previous_selector()
        require(
            current_selector == self.prestate_payloads["SELECTOR"]
            and payload["generation"] == PREVIOUS_GENERATION,
            "previous_selector_changed_during_activation",
        )
        telegram_identity = pwd.getpwnam(TELEGRAM_RUNTIME_USER)
        first = inspect_epoch_bundle(Path(PREVIOUS_EPOCH_PATH), expected_file_mode=0o600, expected_parent_mode=0o700, expected_uid=telegram_identity.pw_uid, expected_gid=telegram_identity.pw_gid)
        time.sleep(0.25)
        second = inspect_epoch_bundle(Path(PREVIOUS_EPOCH_PATH), expected_file_mode=0o600, expected_parent_mode=0o700, expected_uid=telegram_identity.pw_uid, expected_gid=telegram_identity.pw_gid)
        bundle_digest = require_same_bundle(first, second)
        require(bundle_digest == snapshot.release_set.rollback["epoch_bundle_digest"], "previous_bundle_changed_during_activation")
        ...
        seal_epoch_bundle(Path(PREVIOUS_EPOCH_PATH), expected_bundle_digest=bundle_digest, source_uid=telegram_identity.pw_uid, source_gid=telegram_gid, sealed_gid=telegram_gid)
        self.bundle_sealed = True
        target_parent = Path(RELEASE_SET_EPOCH_PATH).parent
        require(not target_parent.exists() and not target_parent.is_symlink(), "generation12_epoch_preexisting")
    ...
    def observe_target(self) -> FunctionalObservation:
        ...
        expected_telegram_config_digest = (
            self.prepared.target_telegram_config_digest
            or self.prepared.prestate["files"]["telegram_resume_config"]["sha256"]
        )
        require(
            digest_file(TELEGRAM_CONFIG) == expected_telegram_config_digest,
    ...
    def restore_prestate(self, prestate: ActivationPrestate) -> None:
        ...
        _restore_release_set_prestate(
            RELEASE_SET_PATH,
            self.prestate_payloads["RELEASE_SET"],
            backup_root=self.backup_root,
            core_uid=core_uid,
            telegram_uid=telegram_uid,
        )
        if self.bundle_sealed:
            restore_epoch_bundle_permissions(Path(PREVIOUS_EPOCH_PATH), prestate=self.bundle_prestate, expected_bundle_digest=self.prepared.release_set.rollback["epoch_bundle_digest"])
    ...
    def _observe_prestate_exact(self) -> FunctionalObservation:
        selector, payload = _load_previous_selector()
        require(payload["generation"] == PREVIOUS_GENERATION and digest_bytes(selector) == self.prepared.release_set.rollback["selector_digest"], "rollback_selector_rejected")
        ...
        if self.prestate_payloads.get("RELEASE_SET") is not None:
            inspect_release_set_acl(
                RELEASE_SET_PATH,
                core_uid=pwd.getpwnam("myuna").pw_uid,
                telegram_uid=pwd.getpwnam(TELEGRAM_RUNTIME_USER).pw_uid,
...
def _attempt_count() -> int:
    ...
    require(isinstance(payload, dict) and payload.get("schema") == "myuna.p07-generation12-component-attempt-ledger.v1" and type(payload.get("attempts")) is int and 0 <= payload["attempts"] <= 2, "attempt_ledger_rejected")
...
def _consume_attempt(plan_digest: str) -> int:
    ...
    count += 1
    atomic_write(ATTEMPT_LEDGER, canonical({"attempts": count, "last_plan_sha256": plan_digest, "schema": "myuna.p07-generation12-component-attempt-ledger.v1"}), mode=0o600)
    return count
...
def activate(
    ...
    *,
    ...
    if phase_f_selected_target(Path(__file__).resolve().parent):
        raise Generation12ActivationRejected("phase_f_canonical_owner_required")
    require(coordinated, "combined_coordinator_required")
    ...
    try:
        result = AtomicReleaseSetTransaction(Generation12LiveBackend(prepared, backup_root)).run()
        receipt = {
            "attempt": attempt,
...
def main() -> int:
    ...
    try:
        ...
        ExternalEpochBundleRejected,
        ExternalEpochV3Rejected,
        Generation12ActivationRejected,
        OSError,
        ReleaseSetAclRejected,
        ReleaseSetActivationRejected,
        sqlite3.Error,
