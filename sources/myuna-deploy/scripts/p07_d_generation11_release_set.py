...
GENERATION = RELEASE_SET_GENERATION_11
...
class Generation11ReleaseSetRejected(RuntimeError):
...
def _require(condition: bool, code: str) -> None:
    if not condition:
        raise Generation11ReleaseSetRejected(code)
...
def selector_payload(previous_bundle_digest: str) -> dict[str, object]:
    _require(
        ...
        "generation11_bundle_digest_rejected",
    )
    return {
