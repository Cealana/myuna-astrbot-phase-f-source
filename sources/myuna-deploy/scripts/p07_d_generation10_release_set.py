...
GENERATION = RELEASE_SET_GENERATION_10
...
class Generation10ReleaseSetRejected(RuntimeError):
...
def _require(condition: bool, code: str) -> None:
    if not condition:
        raise Generation10ReleaseSetRejected(code)
...
def selector_payload(previous_bundle_digest: str) -> dict[str, object]:
    _require(
        ...
        "generation10_bundle_digest_rejected",
    )
    return {
