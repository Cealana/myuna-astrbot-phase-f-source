...
import activate_p07_local_repair_bounded_v1 as activation  # noqa: E402


class P07LocalRepairBoundedActivationTests(unittest.TestCase):
    def test_release_evidence_and_manifest_are_content_free(self) -> None:
        ...
        self.assertEqual(verification["core_tests"], 656)
        self.assertEqual(
            verification["local_repair_projection"],
            "system-plus-current-exchange-v1",
        )
        self.assertEqual(verification["local_repair_projected_message_count"], 4)
    ...
    def test_plan_is_core_only_and_keeps_gateway_timeout(self) -> None:
        plan = json.loads(activation.plan_bytes())
        self.assertEqual(
            plan["target"]["local_repair_projection"],
            "system-plus-current-exchange-v1",
        )
        self.assertEqual(plan["target"]["local_repair_tail_messages"], 3)
        self.assertEqual(
        ...
        self.assertFalse(plan["live_scope"]["gateway_timeout_change"])
    ...
    def test_source_has_no_private_or_active_probe(self) -> None:
        source = (SCRIPTS / "activate_p07_local_repair_bounded_v1.py").read_text(
            encoding="utf-8"
        )
        for forbidden in (
            "/healthz",
        ...
        self.assertIn("candidate_local_repair_probe_rejected", source)
        self.assertIn("restore_binding_and_selector_exact_bytes", source)
        self.assertIn("raw_message_recorded", source)

    def test_activation_reports_third_organic_owner_gate(self) -> None:
