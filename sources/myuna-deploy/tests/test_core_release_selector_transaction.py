from __future__ import annotations

import ast
from copy import deepcopy
from hashlib import sha256
import json
import os
from pathlib import Path
import stat
import sys
import tempfile
import unittest


ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "scripts"))

from core_release_selector import (  # noqa: E402
    canonical_json_bytes,
    load_binding_intent,
    parse_json_document,
    render_guard_dropin,
    render_runtime_binding,
    render_selector_dropin,
    SelectionCandidate,
)
from core_release_selector_transaction import (  # noqa: E402
    ACTIVATION_PLAN_PATH,
    DELETE_LIST_PATH,
    FINAL_PREFIX,
    GUARD_NAME,
    MANIFEST_PATH,
    MIGRATION_EVIDENCE_PATH,
    ROLLBACK_BASE_PATH,
    ROLLBACK_PREFIX,
    RUNTIME_BINDING_PATH,
    SELECTOR_NAME,
    TransactionContractError,
    build_activation_plan,
    build_transaction_payloads,
    digest,
    load_activation_plan,
    transaction_tree_digest,
    validate_transaction_payloads,
)
from install_core_release_selector_transaction import (  # noqa: E402
    TransactionInstallError,
    install_inactive_transaction,
)


R4B_PLAN_DIGEST = "f" * 64


def fixture() -> dict[str, object]:
    intent = load_binding_intent(
        parse_json_document(
            (
                ROOT
                / "config/core-release-selector-v1-binding-intent.json"
            ).read_bytes()
        )
    )
    candidate = SelectionCandidate(selected_release=intent.selected_release)
    selector = render_selector_dropin(candidate).encode("utf-8")
    guard = render_guard_dropin(intent.verifier_script_path).encode("utf-8")
    base = (
        b"[Service]\n"
        b"WorkingDirectory=/srv/myuna/repos/core\n"
        b"Environment=PYTHONPATH=/srv/myuna/repos/core/src\n"
    )
    legacy = (
        b"[Service]\n"
        b"WorkingDirectory=/srv/myuna/releases/core/legacy\n"
        b"Environment=PYTHONPATH=/srv/myuna/releases/core/legacy/src\n"
    )
    feature = b"[Service]\nEnvironment=MYUNA_FEATURE=1\n"
    rollback = {
        "90-legacy.conf": legacy,
        "feature.conf": feature,
    }
    final = {
        GUARD_NAME: guard,
        SELECTOR_NAME: selector,
        "feature.conf": feature,
    }
    writes = {
        GUARD_NAME: guard,
        SELECTOR_NAME: selector,
    }
    plan = build_activation_plan(
        deploy_commit="a" * 40,
        core_commit="b" * 40,
        migration_contract_sha256="c" * 64,
        r3b_plan_digest="d" * 64,
        binding_intent_sha256="e" * 64,
        verifier_sha256=intent.verifier_script_sha256,
        base_template_sha256=digest(base),
        prestate_dropin_sha256={
            name: digest(payload) for name, payload in rollback.items()
        },
        prestate_effective_owner="90-legacy.conf",
        prestate_working_directory="/srv/myuna/releases/core/legacy",
        target_release_path=intent.selected_release.release_path.as_posix(),
        target_tree_sha256=intent.selected_release.tree_sha256,
        target_file_count=intent.selected_release.file_count,
        final_dropin_sha256={
            name: digest(payload) for name, payload in final.items()
        },
        write_dropin_sha256={
            name: digest(payload) for name, payload in writes.items()
        },
        deletes=("90-legacy.conf",),
        gateway_fragment_sha256="1" * 64,
        gateway_dropin_sha256={"gateway.conf": "2" * 64},
    )
    binding = canonical_json_bytes(
        render_runtime_binding(
            intent, approval_plan_digest=digest(plan)
        ).to_payload()
    )
    payloads = build_transaction_payloads(
        activation_plan=plan,
        runtime_binding=binding,
        base_template=base,
        rollback_dropins=rollback,
        final_dropins=final,
        write_dropins=writes,
        deletes=("90-legacy.conf",),
    )
    return {
        "intent": intent,
        "plan": plan,
        "binding": binding,
        "base": base,
        "rollback": rollback,
        "final": final,
        "writes": writes,
        "payloads": payloads,
    }


class ActivationPlanTests(unittest.TestCase):
    def test_activation_plan_is_canonical_and_excludes_recursive_hashes(self) -> None:
        plan = fixture()["plan"]
        loaded = load_activation_plan(plan)
        self.assertEqual(canonical_json_bytes(loaded), plan)
        text = plan.decode("utf-8")
        self.assertNotIn("transaction_tree_sha256", text)
        self.assertNotIn("runtime_binding_sha256", text)
        self.assertNotIn("approval_plan_digest", text)

    def test_activation_plan_rejects_noncanonical_json(self) -> None:
        plan = fixture()["plan"]
        with self.assertRaises(TransactionContractError):
            load_activation_plan(plan + b"\n")

    def test_activation_plan_rejects_identity_drift(self) -> None:
        payload = json.loads(fixture()["plan"].decode("utf-8"))
        payload["status"] = "active"
        with self.assertRaises(TransactionContractError):
            load_activation_plan(canonical_json_bytes(payload))

    def test_activation_plan_rejects_migration_set_drift(self) -> None:
        item = fixture()
        final = dict(item["final"])
        with self.assertRaises(TransactionContractError):
            build_activation_plan(
                deploy_commit="a" * 40,
                core_commit="b" * 40,
                migration_contract_sha256="c" * 64,
                r3b_plan_digest="d" * 64,
                binding_intent_sha256="e" * 64,
                verifier_sha256=item["intent"].verifier_script_sha256,
                base_template_sha256=digest(item["base"]),
                prestate_dropin_sha256={
                    name: digest(payload)
                    for name, payload in item["rollback"].items()
                },
                prestate_effective_owner="90-legacy.conf",
                prestate_working_directory="/legacy",
                target_release_path=item[
                    "intent"
                ].selected_release.release_path.as_posix(),
                target_tree_sha256=item["intent"].selected_release.tree_sha256,
                target_file_count=item["intent"].selected_release.file_count,
                final_dropin_sha256={
                    name: digest(payload) for name, payload in final.items()
                },
                write_dropin_sha256={},
                deletes=("90-legacy.conf",),
                gateway_fragment_sha256="1" * 64,
                gateway_dropin_sha256={"gateway.conf": "2" * 64},
            )


class TransactionPayloadTests(unittest.TestCase):
    def test_transaction_round_trip_is_plan_bound(self) -> None:
        item = fixture()
        evidence = validate_transaction_payloads(item["payloads"])
        binding = parse_json_document(item["binding"])
        self.assertEqual(
            binding["approval_plan_digest"], digest(item["plan"])
        )
        self.assertEqual(evidence.activation_plan_digest, digest(item["plan"]))
        self.assertEqual(
            evidence.transaction_tree_sha256,
            transaction_tree_digest(item["payloads"]),
        )

    def test_transaction_contains_complete_forward_and_rollback_sets(self) -> None:
        item = fixture()
        payloads = item["payloads"]
        self.assertEqual(
            {
                path[len(FINAL_PREFIX) :]
                for path in payloads
                if path.startswith(FINAL_PREFIX)
            },
            set(item["final"]),
        )
        self.assertEqual(
            {
                path[len(ROLLBACK_PREFIX) :]
                for path in payloads
                if path.startswith(ROLLBACK_PREFIX)
            },
            set(item["rollback"]),
        )
        for path in (
            MANIFEST_PATH,
            ACTIVATION_PLAN_PATH,
            RUNTIME_BINDING_PATH,
            ROLLBACK_BASE_PATH,
            MIGRATION_EVIDENCE_PATH,
            DELETE_LIST_PATH,
        ):
            self.assertIn(path, payloads)

    def test_modified_artifact_fails_closed(self) -> None:
        payloads = dict(fixture()["payloads"])
        payloads[RUNTIME_BINDING_PATH] += b"\n"
        with self.assertRaises(TransactionContractError):
            validate_transaction_payloads(payloads)

    def test_extra_artifact_fails_closed(self) -> None:
        payloads = dict(fixture()["payloads"])
        payloads["unexpected.txt"] = b"x"
        with self.assertRaises(TransactionContractError):
            validate_transaction_payloads(payloads)

    def test_missing_artifact_fails_closed(self) -> None:
        payloads = dict(fixture()["payloads"])
        payloads.pop(DELETE_LIST_PATH)
        with self.assertRaises(TransactionContractError):
            validate_transaction_payloads(payloads)

    def test_delete_list_drift_fails_closed(self) -> None:
        payloads = dict(fixture()["payloads"])
        delete_list = json.loads(payloads[DELETE_LIST_PATH].decode("utf-8"))
        delete_list["dropins"] = []
        payloads[DELETE_LIST_PATH] = canonical_json_bytes(delete_list)
        manifest = json.loads(payloads[MANIFEST_PATH].decode("utf-8"))
        manifest["artifacts"][DELETE_LIST_PATH] = sha256(
            payloads[DELETE_LIST_PATH]
        ).hexdigest()
        payloads[MANIFEST_PATH] = canonical_json_bytes(manifest)
        with self.assertRaises(TransactionContractError):
            validate_transaction_payloads(payloads)

    def test_wrong_binding_plan_digest_fails_closed(self) -> None:
        item = fixture()
        wrong_binding = canonical_json_bytes(
            render_runtime_binding(
                item["intent"], approval_plan_digest="0" * 64
            ).to_payload()
        )
        with self.assertRaises(TransactionContractError):
            build_transaction_payloads(
                activation_plan=item["plan"],
                runtime_binding=wrong_binding,
                base_template=item["base"],
                rollback_dropins=item["rollback"],
                final_dropins=item["final"],
                write_dropins=item["writes"],
                deletes=("90-legacy.conf",),
            )

    def test_tree_digest_is_order_independent_and_rejects_unsafe_path(self) -> None:
        payloads = fixture()["payloads"]
        reversed_payloads = dict(reversed(list(payloads.items())))
        self.assertEqual(
            transaction_tree_digest(payloads),
            transaction_tree_digest(reversed_payloads),
        )
        with self.assertRaises(TransactionContractError):
            transaction_tree_digest({"../escape": b"x"})


class InactiveInstallTests(unittest.TestCase):
    def _write_source(
        self, root: Path, payloads: dict[str, bytes]
    ) -> Path:
        source = root / "source"
        source.mkdir()
        for relative, payload in payloads.items():
            target = source / relative
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_bytes(payload)
        return source

    def _roots(self, root: Path) -> tuple[Path, Path]:
        managed = root / "opt/myuna/core-release-selector"
        managed.mkdir(parents=True, mode=0o750)
        os.chown(managed, os.getuid(), os.getgid())
        managed.chmod(0o750)
        return managed / "transactions", managed / "receipts"

    def test_install_is_content_addressed_inactive_and_receipted(self) -> None:
        item = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = self._write_source(root, item["payloads"])
            transaction_root, receipt_root = self._roots(root)
            expected = transaction_tree_digest(item["payloads"])
            result = install_inactive_transaction(
                R4B_PLAN_DIGEST,
                source_root=source,
                expected_transaction_digest=expected,
                transaction_root=transaction_root,
                receipt_root=receipt_root,
                uid=os.getuid(),
                gid=os.getgid(),
            )
            self.assertTrue(result["transaction_created"])
            self.assertTrue(result["receipt_created"])
            self.assertFalse(result["runtime_changed"])
            destination = transaction_root / expected
            self.assertEqual(stat.S_IMODE(destination.stat().st_mode), 0o550)
            self.assertEqual(
                stat.S_IMODE(
                    (destination / MANIFEST_PATH).stat().st_mode
                ),
                0o440,
            )
            receipt = receipt_root / f"{R4B_PLAN_DIGEST}.json"
            self.assertEqual(stat.S_IMODE(receipt.stat().st_mode), 0o440)

    def test_install_is_idempotent(self) -> None:
        item = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = self._write_source(root, item["payloads"])
            transaction_root, receipt_root = self._roots(root)
            expected = transaction_tree_digest(item["payloads"])
            arguments = dict(
                source_root=source,
                expected_transaction_digest=expected,
                transaction_root=transaction_root,
                receipt_root=receipt_root,
                uid=os.getuid(),
                gid=os.getgid(),
            )
            install_inactive_transaction(R4B_PLAN_DIGEST, **arguments)
            second = install_inactive_transaction(
                R4B_PLAN_DIGEST, **arguments
            )
            self.assertFalse(second["transaction_created"])
            self.assertFalse(second["receipt_created"])

    def test_existing_tree_drift_fails_closed(self) -> None:
        item = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = self._write_source(root, item["payloads"])
            transaction_root, receipt_root = self._roots(root)
            expected = transaction_tree_digest(item["payloads"])
            arguments = dict(
                source_root=source,
                expected_transaction_digest=expected,
                transaction_root=transaction_root,
                receipt_root=receipt_root,
                uid=os.getuid(),
                gid=os.getgid(),
            )
            install_inactive_transaction(R4B_PLAN_DIGEST, **arguments)
            target = transaction_root / expected / MANIFEST_PATH
            target.chmod(0o640)
            target.write_bytes(target.read_bytes() + b"\n")
            target.chmod(0o440)
            with self.assertRaises(TransactionInstallError):
                install_inactive_transaction(
                    R4B_PLAN_DIGEST, **arguments
                )

    @unittest.skipUnless(hasattr(os, "symlink"), "symlink support required")
    def test_symlinked_source_fails_closed(self) -> None:
        item = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = self._write_source(root, item["payloads"])
            victim = next(path for path in source.rglob("*") if path.is_file())
            content = victim.read_bytes()
            victim.unlink()
            elsewhere = root / "elsewhere"
            elsewhere.write_bytes(content)
            victim.symlink_to(elsewhere)
            transaction_root, receipt_root = self._roots(root)
            with self.assertRaises(TransactionInstallError):
                install_inactive_transaction(
                    R4B_PLAN_DIGEST,
                    source_root=source,
                    expected_transaction_digest=transaction_tree_digest(
                        item["payloads"]
                    ),
                    transaction_root=transaction_root,
                    receipt_root=receipt_root,
                    uid=os.getuid(),
                    gid=os.getgid(),
                )

    def test_invalid_or_mismatched_digests_fail_closed(self) -> None:
        item = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = self._write_source(root, item["payloads"])
            transaction_root, receipt_root = self._roots(root)
            for approval, expected in (
                ("A" * 64, transaction_tree_digest(item["payloads"])),
                (R4B_PLAN_DIGEST, "0" * 64),
            ):
                with self.subTest(approval=approval, expected=expected):
                    with self.assertRaises(TransactionInstallError):
                        install_inactive_transaction(
                            approval,
                            source_root=source,
                            expected_transaction_digest=expected,
                            transaction_root=transaction_root,
                            receipt_root=receipt_root,
                            uid=os.getuid(),
                            gid=os.getgid(),
                        )

    def test_existing_receipt_drift_fails_closed(self) -> None:
        item = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = self._write_source(root, item["payloads"])
            transaction_root, receipt_root = self._roots(root)
            expected = transaction_tree_digest(item["payloads"])
            arguments = dict(
                source_root=source,
                expected_transaction_digest=expected,
                transaction_root=transaction_root,
                receipt_root=receipt_root,
                uid=os.getuid(),
                gid=os.getgid(),
            )
            install_inactive_transaction(R4B_PLAN_DIGEST, **arguments)
            receipt = receipt_root / f"{R4B_PLAN_DIGEST}.json"
            receipt.chmod(0o640)
            receipt.write_bytes(receipt.read_bytes() + b"\n")
            receipt.chmod(0o440)
            with self.assertRaises(TransactionInstallError):
                install_inactive_transaction(
                    R4B_PLAN_DIGEST, **arguments
                )


class StaticSafetyTests(unittest.TestCase):
    def test_pure_contract_has_no_side_effect_api(self) -> None:
        path = ROOT / "scripts/core_release_selector_transaction.py"
        source = path.read_text(encoding="utf-8")
        tree = ast.parse(source)
        imported: set[str] = set()
        calls: set[str] = set()
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                imported.update(
                    alias.name.split(".", 1)[0] for alias in node.names
                )
            elif isinstance(node, ast.ImportFrom) and node.module:
                imported.add(node.module.split(".", 1)[0])
            elif isinstance(node, ast.Call) and isinstance(
                node.func, ast.Name
            ):
                calls.add(node.func.id)
        self.assertTrue(
            {
                "os",
                "subprocess",
                "socket",
                "requests",
                "urllib",
                "shutil",
            }.isdisjoint(imported)
        )
        self.assertTrue(
            {"open", "system", "popen"}.isdisjoint(calls)
        )

    def test_installer_has_no_process_network_or_active_runtime_api(self) -> None:
        path = (
            ROOT
            / "scripts/install_core_release_selector_transaction.py"
        )
        source = path.read_text(encoding="utf-8")
        tree = ast.parse(source)
        imported: set[str] = set()
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                imported.update(
                    alias.name.split(".", 1)[0] for alias in node.names
                )
            elif isinstance(node, ast.ImportFrom) and node.module:
                imported.add(node.module.split(".", 1)[0])
        self.assertTrue(
            {
                "subprocess",
                "socket",
                "requests",
                "urllib",
            }.isdisjoint(imported)
        )
        self.assertNotIn("/etc/systemd/", source)
        self.assertNotIn(
            "/etc/myuna/core-release-selector/qq.binding.json", source
        )
        self.assertNotIn("daemon-reload", source)


if __name__ == "__main__":
    unittest.main()
