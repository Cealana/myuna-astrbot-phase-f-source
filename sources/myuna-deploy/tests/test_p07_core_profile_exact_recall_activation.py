...
import activate_p07_core_profile_exact_recall_v1 as activation  # noqa: E402


class P07CoreProfileExactRecallActivationTests(unittest.TestCase):
    def test_release_evidence_and_manifest_are_content_free(self) -> None:
        ...
        self.assertEqual(artifact["verification"]["core_tests"], 527)
        self.assertEqual(
            artifact["verification"]["owner_profile_exact_recall"],
            "top-ranked-section-deterministic-v1",
        )
        self.assertFalse(artifact["verification"]["private_content_present"])

    def test_plan_preserves_store_provider_and_channels(self) -> None:
        plan = json.loads(activation.plan_bytes())
        self.assertEqual(
            plan["target"]["owner_profile_exact_recall"],
    ...
    def test_activation_has_no_health_profile_or_provider_probe(self) -> None:
        source = (
            SCRIPTS / "activate_p07_core_profile_exact_recall_v1.py"
        ...
        self.assertIn("candidate_exact_recall_probe_rejected", source)
