...
import activate_p07_d_generation11_v1 as activation
from p07_d_activation_transaction import ReleaseSetActivationRejected
from p07_d_generation11_release_set import canonical
from p07_d_generation11_release_set import selector_payload
...
class Generation11ActivationTests(unittest.TestCase):
    def test_recovery_goal_uses_isolated_backup_and_attempt_ledger(self) -> None:
        self.assertEqual(
            activation.STATE_ROOT,
            Path("/var/lib/myuna-telegram-gateway/p07-generation10-envelope-recovery-v1"),
        )
        self.assertEqual(activation.ATTEMPT_LEDGER.parent, activation.STATE_ROOT)
        self.assertEqual(
            activation.BACKUP_ROOT,
            Path("/var/backups/myuna/p07-generation10-envelope-recovery-v1"),
        )

    def test_recovery_goal_reads_its_own_attempt_ledger(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            successor_root = root / "generation10-envelope-recovery-state"
                ...
                canonical(
                    {
                        ...
                        "schema": "myuna.p07-generation10-envelope-recovery-attempt-ledger.v1",
                    }
                )
            )
            ledger.chmod(0o600)
    ...
    def test_attempt_ledger_is_goal_scoped_and_bounded_to_two(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            activation.Generation11ActivationRejected,
    ...
    def test_generation11_selector_does_not_reuse_failed_paths(self) -> None:
        payload = selector_payload("a" * 64)
        self.assertEqual(activation.GENERATION, 11)
        ...
        self.assertNotIn("external-d-reset-v4/", payload["database_path"])
    ...
    def test_failure_projection_never_copies_untyped_exception_text(self) -> None:
        ...
        self.assertEqual(
            projection,
            {"failure_gate": "generation11_activation_rejected"},
        )
    ...
    @unittest.skipUnless(
        os.environ.get("P07_GENERATION11_CORE_CANDIDATE")
        and os.environ.get("P07_GENERATION11_RUNTIME_CANDIDATE"),
        "explicit immutable candidates required",
    )
    def test_exact_service_identities_can_read_shared_manifest(self) -> None:
        prepared = SimpleNamespace(
            core_candidate=Path(os.environ["P07_GENERATION11_CORE_CANDIDATE"]),
            runtime_candidate=Path(os.environ["P07_GENERATION11_RUNTIME_CANDIDATE"]),
