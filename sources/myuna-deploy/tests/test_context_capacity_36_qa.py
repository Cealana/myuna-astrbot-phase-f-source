...
from context_capacity_36_qa import (  # noqa: E402
    PROFILE_MAX_CHARACTERS,
    PROFILE_MAX_MESSAGES,
    PROFILE_MIN_HTTP_BODY_BYTES,
...
class ContextCapacity36QATests(unittest.TestCase):
    def test_exact_storage_request_order_and_recall_positions(self) -> None:
        ...
        self.assertEqual(len(request), 35)
        ...
        self.assertIn("FIRST=雾蓝纸鹤", request[0]["content"])
        self.assertTrue(any("MIDDLE=珊瑚时钟" in item["content"] for item in request))
    ...
    def test_character_budget_trims_oldest_complete_pairs(self) -> None:
        ...
        for turn in range(7):
            request = history.request_messages("synthetic-long", f"u{turn}-" + "你" * 3997)
            history.commit_reply("synthetic-long", request, f"a{turn}-" + "界" * 3997)
        stored = history.store.load("synthetic-long")
        self.assertEqual(len(stored), 12)
        self.assertLessEqual(sum(len(item["content"]) for item in stored), 48_000)
        self.assertEqual([item["role"] for item in stored], ["user", "assistant"] * 6)
        self.assertTrue(stored[0]["content"].startswith("u1-"))

    def test_session_and_channel_stores_are_isolated(self) -> None:
        telegram = ConversationHistory(36, 48_000)
        qq = ConversationHistory(36, 48_000)
    ...
    def test_failed_request_is_not_committed_or_replayed(self) -> None:
        history = ConversationHistory(36, 48_000)
        ...
        self.assertFalse(any(item["content"] == "must-not-stick" for item in next_request))
        self.assertEqual(next_request[-1]["content"], "after-failure")

    def test_unicode_transport_envelope_has_a_fixed_safe_candidate_limit(self) -> None:
        cjk = core_payload_bytes(maximal_valid_messages("你"))
        escaped = core_payload_bytes(maximal_valid_messages("\x00"))
        self.assertGreater(len(cjk), 65_536)
        self.assertGreater(len(escaped), len(cjk))
        self.assertLessEqual(len(escaped), PROFILE_MIN_HTTP_BODY_BYTES)
    ...
    def test_qq_and_telegram_build_the_same_synthetic_core_payload(self) -> None:
        ...
        for module, channel in (
            ...
            with self.subTest(channel=channel):
                ...
                self.assertLessEqual(len(recorded["body"]), PROFILE_MIN_HTTP_BODY_BYTES)
