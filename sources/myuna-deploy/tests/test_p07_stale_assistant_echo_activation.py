...
import activate_p07_stale_assistant_echo_v1 as activation  # noqa: E402


class P07StaleAssistantEchoActivationTests(unittest.TestCase):
    def test_release_evidence_and_manifest_are_content_free(self) -> None:
        ...
        self.assertEqual(verification["core_tests"], 659)
        self.assertEqual(
            verification["recent_assistant_echo_guard"],
            "local-bounded-lookback-v2",
        )
        self.assertEqual(verification["assistant_echo_lookback"], 3)
        self.assertTrue(verification["exact_repeated_question_exempt"])
    ...
    def test_plan_is_core_only_and_keeps_capacity_and_timeouts(self) -> None:
        ...
        target = plan["target"]
        self.assertEqual(
            target["recent_assistant_echo_guard"],
            "bounded-last-3-assistant-v2",
        )
        self.assertTrue(target["exact_repeated_question_exempt"])
    ...
    def test_source_has_no_private_or_active_probe(self) -> None:
        source = (
            SCRIPTS / "activate_p07_stale_assistant_echo_v1.py"
        ...
        self.assertIn("candidate_stale_echo_wiring_rejected", source)
        self.assertIn("restore_binding_and_selector_exact_bytes", source)
        self.assertIn("raw_message_recorded", source)

    def test_activation_reports_fifth_organic_owner_gate(self) -> None:
