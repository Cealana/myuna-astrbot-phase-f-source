...
import activate_p07_profile_incidental_continuity_v1 as activation  # noqa: E402


class P07ProfileIncidentalContinuityActivationTests(unittest.TestCase):
    def test_release_evidence_and_manifest_are_content_free(self) -> None:
        ...
        self.assertEqual(verification["core_tests"], 647)
        self.assertEqual(
            verification["incidental_profile_enrichment"],
            "recent-complete-turns-v1",
        )
    ...
    def test_plan_is_core_only_and_preserves_private_stores(self) -> None:
        ...
        self.assertEqual(
            plan["target"]["direct_profile_query_projection"],
            "strict-current-turn-v1",
        )
        self.assertEqual(
            plan["target"]["incidental_profile_enrichment"],
            "recent-complete-turns-v1",
        )
        self.assertEqual(
    ...
    def test_source_has_no_private_or_active_probe(self) -> None:
        source = (
            SCRIPTS / "activate_p07_profile_incidental_continuity_v1.py"
        ...
        self.assertIn("candidate_continuity_probe_rejected", source)
        self.assertIn("restore_binding_and_selector_exact_bytes", source)
        self.assertIn("raw_message_recorded", source)

    def test_activation_reports_organic_owner_gate(self) -> None:
