...
import activate_p07_core_profile_context_isolation_v1 as activation  # noqa: E402


class P07CoreProfileContextIsolationActivationTests(unittest.TestCase):
    def test_release_evidence_and_manifest_are_content_free(self) -> None:
        ...
        self.assertEqual(artifact["verification"]["core_tests"], 526)
        self.assertEqual(
            artifact["verification"]["owner_profile_context_isolation"],
            "selected-profile-bounded-v1",
        )
        self.assertFalse(artifact["verification"]["private_content_present"])

    def test_plan_preserves_session_store_and_both_channels(self) -> None:
        plan = json.loads(activation.plan_bytes())
        self.assertEqual(
            plan["target"]["owner_profile_context_isolation"],
    ...
    def test_activation_has_no_health_profile_or_provider_probe(self) -> None:
        source = (
            SCRIPTS / "activate_p07_core_profile_context_isolation_v1.py"
        ...
        self.assertIn("candidate_isolation_probe_rejected", source)
        self.assertIn("raw_message_recorded", source)
        self.assertIn("restore_binding_and_selector_exact_bytes", source)

    def test_activation_reports_owner_retry_gate(self) -> None:
