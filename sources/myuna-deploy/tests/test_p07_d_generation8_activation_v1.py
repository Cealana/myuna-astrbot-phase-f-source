...
import activate_p07_d_generation8_v1 as activation
from p07_d_activation_transaction import ReleaseSetActivationRejected
from p07_d_generation8_release_set import canonical
from p07_d_generation8_release_set import selector_payload
from tests.test_p07_d_release_set_transaction_v1 import release_set


class Generation8ActivationTests(unittest.TestCase):
    def test_successor_reads_shared_goal_attempt_without_reusing_state_root(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            legacy_root = root / "generation7-goal"
            successor_root = root / "generation8-state"
            legacy_root.mkdir()
            ledger = legacy_root / "ATTEMPT_LEDGER.json"
            ledger.write_bytes(
                canonical(
                    {
                        "attempts": 1,
                        "last_plan_sha256": "a" * 64,
                        "schema": "myuna.p07-d-generation7-attempt-ledger.v1",
                    }
                )
            )
            ledger.chmod(0o600)
            with patch.object(activation, "STATE_ROOT", successor_root), patch.object(
                ...
                self.assertNotEqual(activation.STATE_ROOT, activation.ATTEMPT_LEDGER.parent)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(activation, "STATE_ROOT", root), patch.object(
                ...
                with self.assertRaisesRegex(
                    activation.Generation8ActivationRejected,
    ...
    def test_generation8_selector_does_not_reuse_failed_paths(self) -> None:
        payload = selector_payload("a" * 64)
        self.assertEqual(activation.GENERATION, 8)
    ...
    def test_failure_projection_never_copies_untyped_exception_text(self) -> None:
        ...
        self.assertEqual(
            projection,
            {"failure_gate": "generation8_activation_rejected"},
        )
    ...
    @unittest.skipUnless(
        os.environ.get("P07_GENERATION8_CORE_CANDIDATE")
        and os.environ.get("P07_GENERATION8_RUNTIME_CANDIDATE"),
        "explicit immutable candidates required",
    )
    def test_exact_service_identities_can_read_shared_manifest(self) -> None:
        prepared = SimpleNamespace(
            core_candidate=Path(os.environ["P07_GENERATION8_CORE_CANDIDATE"]),
            runtime_candidate=Path(os.environ["P07_GENERATION8_RUNTIME_CANDIDATE"]),
