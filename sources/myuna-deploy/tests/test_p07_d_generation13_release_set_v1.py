...
class Generation13ReleaseSetTests(unittest.TestCase):
    def test_selector_uses_reset_v7_and_preserves_generation11_rollback_identity(self) -> None:
        payload = selector_payload("a" * 64)
        self.assertEqual(GENERATION, 13)
        self.assertEqual(PREVIOUS_GENERATION, 11)
        self.assertEqual(payload["generation"], 13)
        self.assertEqual(
            payload["epoch_id"],
    ...
    def test_selector_rejects_unknown_or_malformed_bundle_digest(self) -> None:
        for value in ("", "A" * 64, "g" * 64, "a" * 63, "a" * 65):
            with self.subTest(value=value):
                with self.assertRaisesRegex(Exception, "generation13_bundle_digest_rejected"):
