...
class P07DReleaseSet:
    ...
    def __post_init__(self) -> None:
        ...
        _require(
            epoch_required.issubset(self.epoch)
            and set(self.epoch)
            <= epoch_required | {"projection_policy_version"},
    ...
    def projection_policy_version(self) -> str:
        value = self.epoch.get(
            "projection_policy_version",
            EXTERNAL_PROJECTION_POLICY,
        )
        _require(isinstance(value, str), "release_set_epoch_rejected")
        return value
